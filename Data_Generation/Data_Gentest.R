...
sim.structure2 <- as.data.frame(matrix(ncol = 6 , nrow = 96))
{
       colnames(sim.structure2) <- c("n" , "p" , "eta.x" , "eta.y" , "g" , "h")
       sim.structure2[ , "eta.x"] <- c(rep(c(0.0 , 0.1 , 0.2) , 24) , 
                                       rep(0 , 24))
       sim.structure2[ , "eta.y"] <- c(rep(c(rep(0.0 , 3) , rep(0.1 , 3) , rep(0.2 , 3)) , 8) , 
                                       rep(0 , 24))
       sim.structure2[ , "p"] <- c(rep(c(rep(8 , 9) , rep(30 , 9)) , 4) , 
                                   rep(c(rep(8 , 3) , rep(30 , 3)) , 4))
       sim.structure2[ , "n"] <- c(rep(25 , 18) , rep(50 , 18) , rep(100 , 18) , rep(200 , 18) , 
                                   rep(25 , 6) , rep(50 , 6) , rep(100 , 6) , rep(200 , 6))
       sim.structure2[ , "g"] <- c(rep(0 , 72) , rep(c(0.2 , 0.0 , 0.2) , 8))
       sim.structure2[ , "h"] <- c(rep(0 , 72) , rep(c(0.0 , 0.2 , 0.2) , 8))
}
View(sim.structure2)
#generate repped conditions dataframe
sim.structure.repped2 <- as.data.frame(matrix(ncol = 56 , nrow = (96*1000)))
colnames(sim.structure.repped2) <- c("n" , "p" , "eta.x" , "eta.y" , "g" , "h" , 
...
for(i in 1:nrow(sim.structure2)) {
       sim.structure.repped2[ ((1000*(i - 1)) + 1): (1000*i), (1:6)] <- 
              purrr::map_dfr(seq_len(1000) , ~sim.structure2[i , ])
}
...
sim.structure.repped2[ , 7:56] <- seeds

head(sim.structure.repped2)
...
data.full <- sim.structure.repped2 %>%   
       pmap(data.gen)
