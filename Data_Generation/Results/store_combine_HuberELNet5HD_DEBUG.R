#load data
SNCDHuberELNet5HD.debug <- readRDS("/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Full_results/Huberelnet5_HD_DEBUG.RData")
...
for(i in 1:length(SNCDHuberELNet5HD.debug)) { 
  #iteration tracker
  cat("i = " , i , "\n")
  #fill error list
  SNCDHuberELNet5.error[[i]] <- list(error = SNCDHuberELNet5HD.debug[[i]]$error , 
                                 condition = as.data.frame(unlist(SNCDHuberELNet5HD.debug[[i]]$result$condition) , 
                                                           n = n , p = p , 
                                                           eta.x = eta.x , eta.y = eta.y , 
                                                           g = g , h = h , seed = seed))
  #fill in results if results aren't NULL from safely()
  SNCDHuberELNet5.result[[i]] <- SNCDHuberELNet5HD.debug[[i]]$result
  #fill final list
  if(!is.null(SNCDHuberELNet5HD.debug[[i]]$result)) {
    SNCDHuberELNet5.final[[i]] <- SNCDHuberELNet5HD.debug[[i]]$result$important
  } else {
...
for(i in 1:length(SNCDHuberELNet5.final)) {
  if(is.null(SNCDHuberELNet5.final[[i]]$error)) {
    ...
    cat("error at i = " , i , "\n")
    SNCDHuberELNet5.maindf[i , 1:7] <- HD.data_debug[[i]]$conditions
...
saveRDS(SNCDHuberELNet5.error , "/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Error_Storage/SNCDHuberELNet5oHD_error_DEBUG.RData")
saveRDS(SNCDHuberELNet5.final , "/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/MainResults_Storage/SNCDHuberELNet5HD_resultmain_DEBUG.RData")
saveRDS(SNCDHuberELNet5.maindf , "/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/MainResults_Storage/SNCDHuberELnet5HD_resultDF_DEBUG.RData")
