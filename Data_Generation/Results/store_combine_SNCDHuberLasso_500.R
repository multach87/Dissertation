#load data
SNCDHuberLasso.HALF <- readRDS("/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Full_results/SNCDHuberLasso_500.RData")
...
for(i in 1:length(SNCDHuberLasso.HALF)) { 
  #iteration tracker
  cat("i = " , i , "\n")
  #fill error list
  SNCDHuberLasso.error[[i]] <- list(error = SNCDHuberLasso.HALF[[i]]$error , 
                                 condition = as.data.frame(unlist(SNCDHuberLasso.HALF[[i]]$result$condition) , 
                                                           n = n , p = p , 
                                                           eta.x = eta.x , eta.y = eta.y , 
                                                           g = g , h = h , seed = seed))
  #fill in results if results aren't NULL from safely()
  SNCDHuberLasso.result[[i]] <- SNCDHuberLasso.HALF[[i]]$result
  #fill final list
  if(!is.null(SNCDHuberLasso.HALF[[i]]$result)) {
    SNCDHuberLasso.final[[i]] <- SNCDHuberLasso.HALF[[i]]$result$important
  } else {
...
saveRDS(SNCDHuberLasso.error , "/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Error_Storage/SNCDHuberLasso_error_500.RData")
saveRDS(SNCDHuberLasso.final , "/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/MainResults_Storage/SNCDHuberLasso_resultmain_500.RData")
