#load data
SNCDHuberELNet5.HALF <- readRDS("/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Full_results/Huberelnet5_500.RData")
...
for(i in 1:length(SNCDHuberELNet5.HALF)) { 
  #iteration tracker
  cat("i = " , i , "\n")
  #fill error list
  SNCDHuberELNet5.error[[i]] <- list(error = SNCDHuberELNet5.HALF[[i]]$error , 
                                 condition = as.data.frame(unlist(SNCDHuberELNet5.HALF[[i]]$result$condition) , 
                                                           n = n , p = p , 
                                                           eta.x = eta.x , eta.y = eta.y , 
                                                           g = g , h = h , seed = seed))
  #fill in results if results aren't NULL from safely()
  SNCDHuberELNet5.result[[i]] <- SNCDHuberELNet5.HALF[[i]]$result
  #fill final list
  if(!is.null(SNCDHuberELNet5.HALF[[i]]$result)) {
    SNCDHuberELNet5.final[[i]] <- SNCDHuberELNet5.HALF[[i]]$result$important
  } else {
