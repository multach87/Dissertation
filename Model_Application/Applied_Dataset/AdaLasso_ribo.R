...
adalasso.sim.fnct <- function(data) {
       ...
       for(i in 1:length(nu.try)) {
         ...
         adalasso.model <- cv.glmnet(X , Y , family = "gaussian" ,
                                     lambda = lambda.try , 
                                     penalty.factor = 1 / abs(best.ridge.coefs)^nu.try[i] , alpha = 1.0)
...
saveRDS(adalasso.full , "/Users/Matt/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Applied_Storage/adalasso_ribo.RData")
