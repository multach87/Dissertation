       ...
       for (tt in 1:length(lambda.gamma.try))
       {
         ...
         for (jj in 1:K)
         {
           ...
           while(tol>1e-6 & n.iter<100) 
           {
             ...
             cat("hello \n")
             cat("sd(Y.train.new) = " , sd(Y.train.new) , "\n")
             cat("gamma.est.cv = " , gamma.est.cv , "\n")
             cat("Y.train.new = " , Y.train.new , "\n")
...
oslassoPLUS.full <- ribo[1:2] %>%   
       map(safely(oslassoPLUS.sim.fnct))

saveRDS(oslassoPLUS.full , "/Users/Matt/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Applied_Storage/oslassoPLUS_ribo.RData")
