...
adalasso.HD.HALF <- readRDS("/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Full_results/adalasso_HD_500.RData")
...
for(i in 1:length(adalasso.HD.HALF)) { 
  #iteration tracker
  cat("i = " , i , "\n")
  #fill error list
  adalasso.error[[i]] <- list(error = adalasso.HD.HALF[[i]]$error , 
                              condition = as.data.frame(unlist(adalasso.HD.HALF[[i]]$condition) , 
                                                        n = n , p = p , 
                                                        eta.x = eta.x , eta.y = eta.y , 
                                                        g = g , h = h , seed = seed))
  #fill in results if results aren't NULL from safely()
  adalasso.result[[i]] <- adalasso.HD.HALF[[i]]$result
  #fill final list
  if(!is.null(adalasso.HD.HALF[[i]]$result)) {
    adalasso.final[[i]] <- adalasso.HD.HALF[[i]]$result$important
  } else {
...
for(i in 1:length(adalasso.final)) {
  if(is.null(adalasso.final[[i]]$error)) {
    ...
    cat("error at i = " , i , "\n")
    adalasso.results[i , 1:7] <- HD.data[[i]]$conditions
    adalasso.errors <- c(adalasso.errors , i)
  }
}
mean(adalasso.results[ , "fpr"])
mean(adalasso.results[ , "fpr"] , na.rm = T)
mean(adalasso.results[ , "fnr"])
mean(adalasso.results[ , "fnr"] , na.rm = T)
mean(adalasso.results[ , "mpe"])
