...
HuberLasso.HD.HALF <- readRDS("/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Full_results/HuberLasso_HD_500.RData")
...
for(i in 1:length(HuberLasso.HD.HALF)) { 
  #iteration tracker
  cat("i = " , i , "\n")
  #fill error list
  Huber.error[[i]] <- list(error = HuberLasso.HD.HALF[[i]]$error , 
                           condition = as.data.frame(unlist(HuberLasso.HD.HALF[[i]]$result$condition) , 
                                                     n = n , p = p , 
                                                     eta.x = eta.x , eta.y = eta.y , 
                                                     g = g , h = h , seed = seed))
  #fill in results if results aren't NULL from safely()
  Huber.result[[i]] <- HuberLasso.HD.HALF[[i]]$result
  #fill final list
  if(!is.null(HuberLasso.HD.HALF[[i]]$result)) {
    Huber.final[[i]] <- HuberLasso.HD.HALF[[i]]$result$important
  } else {
...
for(i in 1:length(Huber.final)) {
  if(is.null(Huber.final[[i]]$error)) {
    ...
    cat("error at i = " , i , "\n")
    Huber.results[i , 1:7] <- HD.data[[i]]$conditions
