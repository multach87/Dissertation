...
OSHuberLasso.HD.debug <- readRDS("/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Full_results/OSHuberLasso_HD_DEBUG.RData")
...
for(i in 1:length(OSHuberLasso.HD.debug)) { 
  #iteration tracker
  cat("i = " , i , "\n")
  #fill error list
  oshuberlasso.error[[i]] <- list(error = OSHuberLasso.HD.debug[[i]]$error , 
                                  condition = as.data.frame(unlist(OSHuberLasso.HD.debug[[i]]$condition) , 
                                                            n = n , p = p , 
                                                            eta.x = eta.x , eta.y = eta.y , 
                                                            g = g , h = h , seed = seed))
  #fill in results if results aren't NULL from safely()
  oshuberlasso.result[[i]] <- OSHuberLasso.HD.debug[[i]]$result
  #fill final list
  if(!is.null(OSHuberLasso.HD.debug[[i]]$result)) {
    oshuberlasso.final[[i]] <- OSHuberLasso.HD.debug[[i]]$result$important
  } else {
...
for(i in 1:length(oshuberlasso.final)) {
  if(is.null(oshuberlasso.final[[i]]$error)) {
    ...
    cat("error at i = " , i , "\n")
    oshuberlasso.results[i , 1:7] <- HD.data_DEBUG[[i]]$conditions
