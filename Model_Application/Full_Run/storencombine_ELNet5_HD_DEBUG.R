...
elnet5.HD.debug <- readRDS("/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Full_results/elnet5_HD_DEBUG.RData")
...
for(i in 1:length(elnet5.HD.debug)) { 
  #iteration tracker
  cat("i = " , i , "\n")
  #fill error list
  elnet5.error[[i]] <- list(error = elnet5.HD.debug[[i]]$error , 
                            condition = as.data.frame(unlist(elnet5.HD.debug[[i]]$result$condition) , 
                                                      n = n , p = p , 
                                                      eta.x = eta.x , eta.y = eta.y , 
                                                      g = g , h = h , seed = seed))
  #fill in results if results aren't NULL from safely()
  elnet5.result[[i]] <- elnet5.HD.debug[[i]]$result
  #fill final list
  if(!is.null(elnet5.HD.debug[[i]]$result)) {
    elnet5.final[[i]] <- elnet5.HD.debug[[i]]$result$important
  } else {
...
for(i in 1:length(elnet5.final)) {
  if(is.null(elnet5.final[[i]]$error)) {
    ...
    cat("error at i = " , i , "\n")
    elnet5.results[i , 1:7] <- HD.data_DEBUG[[i]]$conditions
