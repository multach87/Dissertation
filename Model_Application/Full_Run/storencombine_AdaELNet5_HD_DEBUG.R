...
adaelnet5.HD.debug <- readRDS("/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Full_results/adaelnet5_HD_DEBUG.RData")
...
for(i in 1:length(adaelnet5.HD.debug)) { 
  #iteration tracker
  cat("i = " , i , "\n")
  #fill error list
  adaelnet5.error[[i]] <- list(error = adaelnet5.HD.debug[[i]]$error , 
                               condition = as.data.frame(unlist(adaelnet5.HD.debug[[i]]$result$condition) , 
                                                         n = n , p = p , 
                                                         eta.x = eta.x , eta.y = eta.y , 
                                                         g = g , h = h , seed = seed))
  #fill in results if results aren't NULL from safely()
  adaelnet5.result[[i]] <- adaelnet5.HD.debug[[i]]$result
  #fill final list
  if(!is.null(adaelnet5.HD.debug[[i]]$result)) {
    adaelnet5.final[[i]] <- adaelnet5.HD.debug[[i]]$result$important
  } else {
...
for(i in 1:length(adaelnet5.final)) {
  if(is.null(adaelnet5.final[[i]]$error)) {
    ...
    cat("error at i = " , i , "\n")
    adaelnet5.results[i , 1:7] <- HD.data_DEBUG[[i]]$conditions
