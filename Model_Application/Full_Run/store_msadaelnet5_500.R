msadaelnet5.half <- readRDS("/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Full_results/msadaelnet5_500.RData")
...
for(i in 1:length(msadaelnet5.half)) { 
#iteration tracker
       cat("i = " , i , "\n")
#fill error list
       msadaelnet5.error[[i]] <- list(error = msadaelnet5.half[[i]]$error , 
                                   condition = as.data.frame(unlist(msadaelnet5.half[[i]]$result$condition) , 
                                                             n = n , p = p , 
                                                             eta.x = eta.x , eta.y = eta.y , 
                                                             g = g , h = h , seed = seed))
#fill in results if results aren't NULL from safely()
       msadaelnet5.result[[i]] <- msadaelnet5.half[[i]]$result
#fill final list
       if(!is.null(msadaelnet5.half[[i]]$result)) {
              msadaelnet5.final[[i]] <- msadaelnet5.half[[i]]$result$important
       } else {
...
#combine diagnostics
#diagnostics <- data.frame(matrix(ncol = 2 , nrow = length(msadaelnet5.half)))
...
#saveRDS(diagnostics , "/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Diagnostics_Storage/msadaelnet5_diagnostics_500.RData")
