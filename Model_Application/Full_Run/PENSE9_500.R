...
#ebug.data <- readRDS("/Users/Matt Multach/Desktop/Dissertation/Dissertation_Git/Data_Generation/Data_Storage/debug_data_091720.RData")
...
#function
pense9.sim.fnct <- function(data , alpha = 0.9) {
  ...
  #set seed
  seed.pensecv <- data$seeds[ , "seed.13"]
  ...
  pense.cv <- pense_cv(x = X , y = Y , alpha = alpha , 
...
pense9.half <- half.data %>%   
  map(safely(pense9.sim.fnct))

saveRDS(pense9.half , "/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Full_results/pense9_500.RData")
