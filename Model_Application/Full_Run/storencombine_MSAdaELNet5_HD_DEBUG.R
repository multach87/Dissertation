...
for(i in 1:length(msaelnet5.HD.debug)) { 
  #iteration tracker
  cat("i = " , i , "\n")
  #fill error list
  msaelnet5.error[[i]] <- list(error = msaelnet5.HD.debug[[i]]$error , 
                               condition = as.data.frame(unlist(msaelnet5.HD.debug[[i]]$result$condition) , 
                                                         n = n , p = p , 
                                                         eta.x = eta.x , eta.y = eta.y , 
                                                         g = g , h = h , seed = seed))
  #fill in results if results aren't NULL from safely()
  msaelnet5.result[[i]] <- msaelnet5.HD.debug[[i]]$result
  #fill final list
  if(!is.null(msaelnet5.HD.debug[[i]]$result)) {
    msaelnet5.final[[i]] <- msaelnet5.HD.debug[[i]]$result$important
  } else {
