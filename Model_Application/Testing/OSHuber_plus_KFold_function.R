...
#OS-Huber + K-fold function
OSH.lasso.plus <- function(X,Y,lambda.lasso.try,k=1.5){
       n<-length(Y)
       Y.orgn<- Y
       model.for.cv<- cv.glmnet(X, Y, family="gaussian",lambda=lambda.lasso.try)
       model.est <- H.lasso(X,Y,lambda.lasso.try,k)
...
OSH.lasso.cv.plus<-function(X,Y,lambda.lasso.try,k=1.5,lambda.gamma.try){
       ...
       for (tt in 1:length(lambda.gamma.try))
       {
              ...
              for (jj in 1:K)
              {
                      ...
                      sample.out.index <- which(X.new[ , "subset"] == jj) #sample.out.index<- (1+n.cv*(jj-1)):(n.cv*(jj))
                      ...
                      X.train<- X.new[X.new[ , "subset"] != subset , -ncol(X.new)] #X.train<- X[-sample.out.index,]
                      Y.train<- Y.new[Y.new[ , 2] != subset , 1] #Y.train<- Y[-sample.out.index]
                      X.test<- X.new[X.new[ , "subset"] == subset , -ncol(X.new)] #X.test<- X[sample.out.index,]
                     ...
                     while(tol>1e-4 & n.iter<51) 
                     {
                            ...
                            Y.train.new <- Y.train - gamma.est.cv
                            #model.train.temp<- H.lasso(X.train,Y.train.new,c(lambda.lasso.opt,lambda.lasso.opt),k=k)
                            model.train.temp<- H.lasso(X.train,Y.train.new,lambda.lasso.try = seq(0.01,0.6,length.out=100),k=k)
                            beta.post <- model.train.temp$coefficient
                            tol<- sum((beta.pre-beta.post)^2)
                            n.iter<- n.iter+1
                            beta.pre<- beta.post
       ...
       while(tol>1e-6 & n.iter<100) 
       {
              ...
              model.opt<- H.lasso(X,Y.new2,c(lambda.lasso.opt,lambda.lasso.opt),k=k)
              beta.post <-  model.opt$coefficient
              tol<- mean((beta.pre-beta.post)^2)
              n.iter<- n.iter+1
              beta.pre<- beta.post
...
OSHuber.model
set.seed(501)
OSHuber.modelPLUS <- OSH.lasso.plus(X = X , Y = Y , lambda.lasso.try = lambda.lasso.try)
OSHuber.modelPLUS

#testing with CV
set.seed(501)
OSHuber.model.cv <- OSH.lasso.cv(X = X , Y = Y , 
                                 lambda.lasso.try = lambda.lasso.try ,
                                 lambda.gamma.try = lambda.gamma.try)
OSHuber.model.cv
set.seed(501)
OSHuber.model.cvPLUS <- OSH.lasso.cv.plus(X = X , Y = Y , 
                                    lambda.lasso.try = lambda.lasso.try ,
                                    lambda.gamma.try = lambda.gamma.try)
OSHuber.model.cvPLUS
