...
ladlasso.testing10 <- testing10.data %>%   
...
for(i in 1:length(ladlasso.testing10)) { 
       #iteration tracker
       cat("i = " , i , "\n")
       #fill error list
       ladlasso.error[[i]] <- list(error = ladlasso.testing10[[i]]$error , 
                                   condition = as.data.frame(unlist(testing10.data[[i]]$condition) , 
                                                             n = n , p = p , 
                                                             eta.x = eta.x , eta.y = eta.y , 
                                                             g = g , h = h , seed = seed))
       #fill in results if results aren't NULL from safely()
       ladlasso.result[[i]] <- ladlasso.testing10[[i]]$result
       #fill final list
       if(!is.null(ladlasso.testing10[[i]]$result)) {
              ladlasso.final[[i]] <- ladlasso.testing10[[i]]$result$important
       } else {
...
#combine diagnostics
diagnostics <- data.frame(matrix(ncol = 4 , nrow = length(testing10.data)))
