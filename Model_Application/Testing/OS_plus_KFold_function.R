...
#OS Lasso function with custom KFold subsetting function
jj <- 1
n <- length(Y)
K <- 5
n.cv <- n / K 
...
X.train2<- X.new[X.new[ , "subset"] != subset , -ncol(X.new)]
Y.train2<- Y[Y.new[ , 2] != subset , ]
X.test2<- X.new[X.new[ , "subset"] == subset , -ncol(X.new)]
FALSE %in% (which(X.new[ , "subset"] == jj) == which(Y.new[ , 2] == jj))
