...
#load data
HD.n200p1000 <- readRDS("/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Split_by_n/HD_n2001000.RData")

rq.fit.lasso2 <- function (x, y, tau = 0.5, lambda = 1, beta = 0.99995, eps = 1e-06) 
{
        n <- length(y)
        p <- ncol(x)
        if (n != nrow(x)) 
...
ladlasso.sim.fnct2 <- function(data) {
       ...
       for(i in 1:length(nu.try)) {
              ...
              for (k in 1:100){
                     rqfit <- rq.fit.lasso2(X , jitter(Y) , lambda = lambda.try[k] * weights)
...
test12 <- ladlasso.sim.fnct2(HD.n200p1000[[1]])
test13 <- ladlasso.sim.fnct2(HD.n200p1000[[1]])

#run across full dataset
ladlasso.HD.n200p190 <- HD.n200p190 %>%   
       map(safely(ladlasso.sim.fnct))

saveRDS(ladlasso.HD.n200p190 , "/Users/Matt Multach/Dropbox/USC_Grad2/Courses/Dissertation/Dissertation_Git/Data_Storage/Full_results/ladlasso_HD_n200p190_500_2.RData")
